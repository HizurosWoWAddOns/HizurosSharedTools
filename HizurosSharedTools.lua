...
local function Supporter_AddStyle1(addon,credit)
	for s=1, #supporter, 4 do
		local reasons = {};
		for r=3,1,-1 do -- special, donation, locale
			local rIndex,objs = s+r,false;
			if supporter[rIndex] then
				objs = thisAddOn(addon,supporter[rIndex]);
			end
			if objs then
				if r==1 then
					tinsert(reasons,L["- For translations"].." ("..Supporter_GetLanguages(objs)..")");
				elseif r==2 then
					tinsert(reasons,L["- For donation"].." ("..Supporter_GetDonationPlatforms(objs)..")");
				else--if type(objs)=="table" then
					tAppendAll(reasons,objs);
				end
			end
		end
		if #reasons>0 then
			local entry = CopyTable(supporterTemplate);
			entry.order = 200+s;
			--
			local name = Supporter_GetName(supporter[s]);
			entry.args.label.name = name;
			entry.args.reasons.name = table.concat(reasons,"|n|t");
			credit.args["entry-"..s] = entry;
		end
	end
	return true;
end

local function Supporter_AddStyle2(addon,credit)
		...
		for r=3,1,-1 do
			...
			if objs and not credit.args["header-"..r] then
				...
				if not firstSection then
					credit.args["saparator-"..r] = CopyTable(headerTemplate);
					credit.args["saparator-"..r].order = rInvert*100+1;
				else
					firstSection = false;
				end
				--]]
				...
				credit.args["header-"..r].name = prefix .. C("cyan",(r==1 and L["ThxLocale"]) or (r==2 and L["ThxSupport"]) or L["ThxSpecial"]);
				if prefix=="" then
					prefix = "|n";
				end
				hasNoEntries = false
...
function lib.AddCredit(addon,creditSection,style)
	...
	-- add supporter
	--Supporter_AddStyle1(addon,credit);
	local isEmpty = Supporter_AddStyle2(addon,credit);
...
--== expand on show hook for BlizzOptions ==--
